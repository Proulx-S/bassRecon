function [outName,cropRange,img] = recon(datFile,datNoiseFile,datPhaseFile,coilMethod,cropRange,verbose)
...
if exist('datPhaseFile','var') && ~isempty(datPhaseFile)
    if ~datPhaseFlag
        [~,~,imgPhase] = recon([],[],datPhaseFile,coilMethod,cropRange,0);
    else
        outName   = [];
        cropRange = [];

        X = permute(imgPhase,[11 3 1 2 4 5 6 7 8 9 10]);
...
if size(img,7)>1
    ...
    X = permute(imgPhase(:,:,:,:,:,:,end-1,:,:,:,:,:,:,:,:,:),[11 3 1 2 4 5 6 7 8 9 10]);
    X = cat(2,real(X),imag(X));
    stats = T2Hot1(X)
    stats.P<0.05
    ...
    figure('MenuBar', 'none','ToolBar', 'none');
    imagesc(angle(imgPhase(:,:,end-1)),[-pi pi]); axis image off; colormap hsv; drawnow;
    imagesc(abs(imgPhase(:,:,end-1))); axis image off; colormap gray; drawnow;
